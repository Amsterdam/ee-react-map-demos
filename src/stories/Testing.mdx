...
Each demo in this project has tests for each file. Test files are stored in the same directories, with `.test` suffixed to the filename. You can find these files in the <a href="https://github.com/Amsterdam/ee-react-map-demos/tree/develop/src/pages" target="_blank" className="sbdocs sbdocs-a">GitHub repository</a> in the `src/pages` directory.

## Why is testing maps difficult?

Testing maps in SPA (Single Page Application) JavsScript applications is challenging. This is because testing engines (for example, <a href="https://jestjs.io/" target="_blank" className="sbdocs sbdocs-a">Jest</a> or <a href="https://vitest.dev/" target="_blank" className="sbdocs sbdocs-a">Vitest</a>) do not use native browsers environments. They often use a virtualized DOM (Document Object Model) engine, for example, <a href="https://github.com/jsdom/jsdom" target="_blank" className="sbdocs sbdocs-a">jsdom</a> or <a href="https://github.com/capricorn86/happy-dom" target="_blank" className="sbdocs sbdocs-a">Happy DOM</a>.
