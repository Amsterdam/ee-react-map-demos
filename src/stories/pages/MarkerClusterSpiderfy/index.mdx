...
1. The React component
    1. [MarkerClusterSpiderfy.tsx](#1-markerclusterspidertsx) (*This is based on the [BaseMap component example](../?path=/docs/react-baselayer--docs) so includes a dependency on [`utils/getCrsRd`](../?path=/docs/react-baselayer--docs#1-getcrsrdts).*)
2. Data handling
    1. [getMapData.ts](#1-getmapdatats)
    2. [processFeatures.ts](#2-processfeaturests)
3. The CSS styles
    1. [styles.module.css](#1-stylesmodulecss)
4. Types for data (typescript only)
    1. [types.ts](#1-typests) *(These have been kept minimal and are primarily for examples.)*
5. Data
    1. [data.json](#1-datajson) *(This static data should be replaced by a fetch call or other static data)*
6. Map and cluster options and styles
    1. [mapStyles.ts](#1-mapstylests)
7. Utils
    1. [utils/roundNumber.ts](#1-utilsroundnumberts)
    2. [utils/createClusterShapes.ts](#2-utilscreateclustershapests)
    3. [utils/toGeoJSON.ts](#3-utilstogeojsonts)
    4. [utils/toBoundsLiteral.ts](#4-utilstoboundsliteralts)

## Usage

### 1. The React component

#### 1. MarkerClusterSpiderfy.tsx
