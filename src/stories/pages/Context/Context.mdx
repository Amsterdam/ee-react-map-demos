...
- Using Context with React involves two methods: `createContext` and `useContext`.

- Combined with TypeScript, definitions and undefined/null properties can make things confusing. Therefore, if you ensure the context instance is defined (with the help TypeScript's [`NonNullable`](https://www.typescriptlang.org/docs/handbook/utility-types.html#nonnullabletype)). For example:
